...
def calculate_priority_proportion(df_long):
    ...
    for date in df_long['Date'].unique():
        ...
        totalind = df_long_date[df_long_date['Category'] == 'total_individuals']['Number']
        priorityind = df_long_date[df_long_date['Category'] == 'priority_individuals']['Number']
        if totalind.empty or priorityind.empty:
            continue
        else:
            totalind = df_long_date[df_long_date['Category'] == 'total_individuals']['Number'].values[0]
            priorityind = df_long_date[df_long_date['Category'] == 'priority_individuals']['Number'].values[0]
            ...
            totalapp = df_long_date[df_long_date['Category'] == 'total_applications']['Number'].values[0]
            priorityapp = df_long_date[df_long_date['Category'] == 'priority_applications']['Number'].values[0]
...
def nonpriority(df_long):
    ...
    for date in df_long['Date'].unique():
        ...
        if totalind.empty or priorityind.empty:
            ...
            totalind = df_long_date[df_long_date['Category'] == 'total_individuals']['Number'].values[0]
            priorityind = df_long_date[df_long_date['Category'] == 'priority_individuals']['Number'].values[0]
            new_row_ind = pd.DataFrame([{'Date': date, 'Category': 'nonpriority_individuals', 'Number': totalind - priorityind}])
            new_rows = pd.concat([new_rows, new_row_ind])

        totalapp = df_long_date[df_long_date['Category'] == 'total_applications']['Number']
        priorityapp = df_long_date[df_long_date['Category'] == 'priority_applications']['Number']
        if totalapp.empty or priorityapp.empty:
            continue
        else:
            totalapp = df_long_date[df_long_date['Category'] == 'total_applications']['Number'].values[0]
            priorityapp = df_long_date[df_long_date['Category'] == 'priority_applications']['Number'].values[0]
            new_row_app = pd.DataFrame([{'Date': date, 'Category': 'nonpriority_applications', 'Number': totalapp - priorityapp}])
            new_rows = pd.concat([new_rows, new_row_app])
    df_long = pd.concat([df_long, new_rows])
    return df_long
